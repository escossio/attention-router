...
def resolve_explicit_recommendation_reply(
    ...
    *,
    ...
    if not candidates:
        return None
    if len(candidates) != 1:
        raise RecommendationReplyError("RECOMMENDATION_REPLY_AMBIGUOUS")

    recommendation_id = (candidates[0].context or {}).get("recommendation_id")
    if not isinstance(recommendation_id, str):
        raise RecommendationReplyError("RECOMMENDATION_ID_MISSING")
    ...
    try:
    ...
    except RecommendationLifecycleError as exc:
        reason_code = str(exc)
        if reason_code == "RECOMMENDATION_SOURCE_HYPOTHESIS_INVALIDATED":
        ...
        raise RecommendationReplyError(reason_code) from exc
