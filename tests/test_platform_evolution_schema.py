...
def test_platform_evolution_migration_waves_form_one_chain():
    ...
    assert scripts.get_heads() == ["0047_artifact_understanding_v1"]
    ...
    assert revisions["0039_human_auth_cont_grant"].down_revision == (
        "0038_human_identity_v1"
    )
    assert revisions["0040_client_device_bootstrap"].down_revision == (
        "0039_human_auth_cont_grant"
    )
    assert revisions["0041_client_session_authority"].down_revision == (
        "0040_client_device_bootstrap"
    )
    assert revisions["0042_client_location_snapshot"].down_revision == (
        "0041_client_session_authority"
    )
    assert revisions["0043_pending_intents_v1"].down_revision == (
        "0042_client_location_snapshot"
    )
    assert revisions["0044_integration_dispatch_v1"].down_revision == (
