...
def test_verify_success_passes_exact_arguments_without_admin(api):
    client, service, session = api
...
def test_domain_error_mapping_matches_contract(api, error, status_code, code):
    client, service, _ = api
    service.error = error
    response = client.post(VERIFY.format(challenge_id=CHALLENGE_ID), json={"id_token": ID_TOKEN})
    assert response.status_code == status_code
    assert response.json() == {"code": code}
    assert str(status_code) in CONTRACT["paths"][VERIFY]["post"]["responses"]
...
def test_runtime_openapi_matches_frozen_contract(api):
    ...
    for path, item in CONTRACT["paths"].items():
        ...
        assert operation["security"] == expected["security"] == []
        assert not any(parameter["in"] == "header" for parameter in operation.get("parameters", []))
        for status_code, response in expected["responses"].items():
            assert operation["responses"][status_code]["content"] == response["content"]
